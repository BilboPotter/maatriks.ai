...
const { assertNoUnresolvedPlaceholders, interpolate } = require('./templates');
...
function getPublicAssetManifest(options) {
  ...
  if (fs.existsSync(scriptsDir)) {
    for (const file of fs.readdirSync(scriptsDir)) {
      ...
      assertNoUnresolvedPlaceholders(interpolated, `script:${file}`);
      const content = minifyJS(interpolated);
